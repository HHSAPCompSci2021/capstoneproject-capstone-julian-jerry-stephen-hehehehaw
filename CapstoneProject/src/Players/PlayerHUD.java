...
//		}
		...
		p.rect((float) (width-width/5.5), height-height/5, (float) (width/5.5 - ratio1 * (player.getInitHealth() - player.getHealth())), height/10);
		...
		if(player.getWeapon().getAmmo() >= 1)
		{
			...
			p.fill(0, 0, 255);
			p.rect((float) (width-width/5.5), (float) (height-height/10.5), (float) (width/5.5 - ratio * (player.getWeapon().getMagSize()-player.getWeapon().getAmmo())), height/10);
		}
		else {
			double ratio = (width/5.5) / player.getWeapon().getReloadTime();
			
			p.fill(255, 0, 0);
			p.rect((float) (width-width/5.5), (float) (height-height/10.5), (float) (ratio * (player.getWeapon().getReloadTime()-player.getWeapon().getReloadCounter())), height/10);
		}
